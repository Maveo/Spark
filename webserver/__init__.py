...
class WebServer(threading.Thread):
    ...
    def __init__(self,
        ...
        for page in api_pages:
            self.app.add_api_route(path=page.path,
                                   endpoint=page.view_func,
                                   methods=page.methods)

        for page in self.dbot.module_manager.api_pages.all():
            self.app.add_api_route(path=f'/{api_base}/{page.path}',
                                   endpoint=self.guild_member_wrapper(page.view_func),
                                   methods=page.methods)
